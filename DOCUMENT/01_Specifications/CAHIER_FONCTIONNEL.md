...
### 1.2 Expression précise des besoins et Utilisateurs Cibles

Le système s'articule autour de deux types d'utilisateurs clés, chacun ayant des rôles et des habilitations strictement différenciés :

#### Administrateur RH (Ressources Humaines) :
- Créer, modifier et archiver les fiches des employés.
- Gérer les contrats de travail.
- Assigner et gérer les éléments variables de paie (primes, heures supplémentaires).
- Générer les bulletins de paie mensuels pour chaque employé.
- Mettre à jour le paramétrage (taux, règles, variables globales).
...
- Consulter et télécharger l'historique de ses bulletins de salaire (PDF).

*(À FAIRE : Insérer le Diagramme Pieuvre ici pour illustrer les interactions des fonctions de service)*

*(À FAIRE : Insérer le Diagramme des Cas d'Utilisation ici pour illustrer ces rôles)*
...
### 3.1 Flux d'Embauche (Gestion Employés)

> 🔗 *Voir le [Workflow détaillé : Gestion des Employés](../06_Workflow/flux/03_gestion_employes_workflow.md)*
...
> 🔗 *Voir le [Workflow détaillé : Génération de Paie](../06_Workflow/flux/02_generation_paie_workflow.md)*
...
## 4. Interfaces Utilisateur et Ergonomie (Analyse Front)

L'interface de GFPS privilégie la clarté pour minimiser la surcharge cognitive. Le design repose sur une hiérarchisation structurée :

### 4.1.1 Palette de Couleurs
* **Bleu Corporate (#1D4ED8) :** Navigation, en-têtes et actions principales. Inspire confiance.
* **Gris Clair (#F3F4F6) :** Fond d'interface et cartes (Card UI).
* **Vert (#10B981) :** Succès et validation.
* **Rouge (#EF4444) :** Erreurs et actions destructrices.
* **Orange (#F59E0B) :** Avertissements.

### 4.1.2 Typographie et Accessibilité
* **Titre H1/H2 :** Délimitation des sections (24px-32px).
* **Corps de texte :** Tableaux et menus (14px-16px).
* Accessibilité : Design responsive, conforme aux normes WCAG.

### 4.1.3 Wireframes (Conception)
*(À FAIRE : Insérer les schémas Wireframes de chaque page ici)*

### 4.2 Détail des Vues (Pages et Fenêtres imbriquées)
