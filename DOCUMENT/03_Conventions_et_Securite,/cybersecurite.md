# Politique et Mesures de Cybersécurité - Projet Mémoire RH
...
- **Chiffrement des mots de passe :** Les mots de passe des utilisateurs ne sont jamais stockés en clair en base de données. Ils sont hachés en utilisant l'algorithme robuste de Spring Security (`PasswordEncoder`, typiquement BCrypt), comme implémenté dans le `AuthController`.
- **Contrôle d'accès basé sur les rôles (RBAC) :** L'application vérifie le rôle de l'utilisateur (ex: `ADMIN`, `EMPLOYEE`) pour restreindre l'accès aux points d'API sensibles (paramètres de paie, gestion de l'établissement, etc.).
- **Protection des points d'entrée :** L'accès à l'API est sécurisé (par exemple via JSON Web Tokens - JWT) pour valider l'identité de l'utilisateur à chaque requête.
