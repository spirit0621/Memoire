...
export function renderEmployees(container) {
    container.innerHTML = `
        <div class="page-header">
            ...
            <button class="btn btn-primary" onclick="ui.openEmployeeModal()"><i class="fa-solid fa-user-plus"></i> New</button>
        </div>
        <div class="filter-bar">
            <div class="flex items-center gap-2" style="flex:1; border: 1px solid var(--slate-200); border-radius: var(--radius-md); padding-left: 1rem; background:white;">
                ...
                <input type="text" id="emp-search" class="form-control" style="border:none; padding-left: 0.5rem;" placeholder="Search by Name..." value="${state.filters.search}">
            </div>
        ...
        document.getElementById('emp-list').innerHTML = filtered.map(e => {
            ...
            return `
                <tr class="animate-fade-in-up">
                            ...
                            <img src="${e.avatar}" class="avatar avatar-sm">
                             <div>
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="badge badge-indigo" style="font-size: 10px; padding: 1px 6px;">${e.position || 'Employee'}</span>
                    ...
                    <td><span class="font-bold text-slate-700">${e.baseSalary.toLocaleString('en-US')} €</span></td>
                    <td style="min-width: 150px;">
                        <div class="leave-tracker">
                            <div class="flex justify-between text-[10px] font-bold text-slate-500 mb-1">
                                <span>${vacationTaken} taken</span>
                                <span>${vacationTotal}d</span>
                            </div>
                    ...
                    <td class="text-right">
                        <div class="flex justify-end gap-1">
                            <button class="btn btn-ghost btn-sm text-blue-600" title="Edit" onclick="ui.openEmployeeModal(${e.id})"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button class="btn btn-ghost btn-sm text-red-600" title="Delete" onclick="api.deleteEmployee(${e.id})"><i class="fa-solid fa-trash-can"></i></button>
                        </div>
                    </td>
                </tr>
            `;
